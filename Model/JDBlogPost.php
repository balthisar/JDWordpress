...
class JDBlogPost extends AppModel
{
	 ...
	 * @return array Structure with the post content and details.
	 **---------------------------------------------------------------------------*/
	...
	/**---------------------------------------------------------------------------*
	 * Retrieves and processes all posts using `$optionsStandard`; if given
	 * `$addedConditions` and/or `$limit`, then those apply as well.
	 *
	 * @param  array  $addedConditions CakePHP conditions array to add.
	 * @param  string $limit           Number of posts to return.
	 * @return array                   Structure with the posts content and details.
	 **---------------------------------------------------------------------------*/
	...
	/**---------------------------------------------------------------------------*
	 * Retrieves posts for the taxonomy $term and $type; for example can
	 * retrieve all posts that have "category" of "news" or all posts with
	 * "post_tag" of "cool".
	 *
	...
	/**---------------------------------------------------------------------------*
	 * Retrieves a list of all posts for the given $taxonomy broken down into
	 * groups by $term.
	 *
	 * @param   string $type Taxonomy type.
	 * @return  array  Structure with the posts content and details.
...
_SQL;
	 ...
	 *
	 * @param   string $slug
	 * @return  string Indicating the `name` field for the slug.
	 **---------------------------------------------------------------------------*/
...
_SQL;
	...
	/**---------------------------------------------------------------------------*
	 * Outputs the SQL database to a file and returns a reference to the filename
		...
		foreach ( $fragments as $node )
		{
			...
			$values = array_values(
				array_filter(
					...
					function ( $item )
					{
						return !in_array(strtolower($item), [ 'geshi', 'geshilinenumbers' ]);
					}
				)
			);
