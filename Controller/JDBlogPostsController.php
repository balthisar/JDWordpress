...
class JDBlogPostsController extends AppController
{
	...
	public $layout = 'default';                         // specify the layout to use.

	/*
		The dump action needs a little setup. This "security" is provided as being better
	 ...
	 */
	public $uses = array();
	public $components = array('JDWordpress.Zip', 'JDWordpress.JDBlogPostData');
	public $helpers = array( 'Rss' => array('className' => 'JDWordpress.FixedRss') );
...
/********************************************************************//**
	...
	@details
		This is really a rather boring index. You'd probably like
		to route to your own index and do something interesting
		there.
 ********************************************************************/
...
/********************************************************************//**
	@brief
		Displays a single post if it exists, then tries to show
		summaries by Author-, Category-, or Tag-slugs respectively,
		and finally x number of posts if the parameter is numeric.

	@details
		This gives an interesting user experience whereby nearly any
		taxonomy, article name, author name, etc. will magically
		work.
 ********************************************************************/
...
/********************************************************************//**
	...
	@details
		We'll do the same as for `view` and magically deliver a feed
		using a taxonomy if it exists:

		If there's no trailing path, then deliver whole-site RSS
		limited	to the past 30 days. Otherwise respectively deliver
		RSS results by treating the trailing path as Author,
		Category, then Tag.
 ********************************************************************/
public function rss()
{
...
/********************************************************************//**
	@brief
		Zips the server's wordpress uploads directory and then
		sends it to the web browser for download.

	@details
		This is just a simple means of getting your remote WP
		data for local development.

		If using the simple built-in security mechanism then this
		action requires a parameter, e.g.,
...
/********************************************************************//**
	@brief
		Dumps the MySQL database that this controller's model
		is attached to and then serve the file as a download
...
/********************************************************************//**
	@brief
		...
		article titles in a \<ul> toc format.

	@details
		For example can provide a list of all 'category' and articles
		for each 'category.'
 ********************************************************************/
